...
run "custom_environment_does_not_fall_back_to_dev" {
  command = plan
  variables {
    github_environment = "release:preview"
  }
  assert {
    condition     = output.github_oidc_subject == "repo:protove@114971169/identity-service@1352173512:environment:release%3Apreview"
    error_message = "Custom GitHub Environment must be used and encoded in the subject."
  }
}
run "invalid_service_name_is_rejected" {
  command = plan
  variables {
    service_name = "Identity"
  }
